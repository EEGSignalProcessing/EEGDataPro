% Modified by Matthew Frehlich, 2016
% 
% tmseeg_pop_prop_modified() - plot the properties of a channel or of an independent
%              component. 
% Usage:
...
%
% Renamed from tmseeg_pop_prop_modified_v2 to eegdatapro_pop_prop_modified_v2 by 
% Ben Schwartzmann 2019

function com = eegdatapro_pop_prop_modified_v2(EEG, typecomp, chanorcomp, winhandle, spec_opt,disp_pack)
global backcolor
com = '';
if nargin < 1
	help pop_prop;
...
if length(chanorcomp) > 1
    for index = chanorcomp
        tmseeg_pop_prop_modified(EEG, typecomp, index, 0, spec_opt);  % call recursively for each chanorcomp
    end;
...
set(gcf,'Position', [pos(1) pos(2)-600+pos(4) 600 550], 'visible', 'on');
...
% plotting topoplot
% -----------------
h = axes('Units','Normalized', 'Position',[-15 43 40 42].*s+q); %[-10 60 40 42].*s+q
...
% plotting erpimage
% -----------------
hhh = axes('Units','Normalized', 'Position',[45 62 48 38].*s+q); % hhh = axes('Units','Normalized', 'Position',[45 62 48 38].*s+q); %image
eeglab_options; 
if EEG.trials > 1
    % put title at top of erpimage
    axis off
    hh = axes('Units','Normalized', 'Position',[30 38 70 60].*s+q); % [45 62 48 38].*s+q
    %EEG.times = linspace(EEG.xmin, EEG.xmax, EEG.pnts);
    if isfield(EEG,'custom') && isfield(EEG.custom, 'times')
        EEG.times = EEG.custom.times/1000;
    else
    ...
    if ERPIMAGELINES > 2   % give up if data too small
      ...
      eegtimes = EEG.times; %linspace(0, erpimageframes-1, EEG.srate/1000);
      if typecomp == 1 % plot channel
           offset = nan_mean(EEG.data(chanorcomp,:));
...
% else
% 	h = axes('units','normalized', 'position',[0 0 95 25].*s+q);
%    % h = axes('units','normalized', 'position',[5 0 95 40].*s+q);
% end;
%h = axes('units','normalized', 'position',[45 5 60 40].*s+q);
h = axes('units','normalized', 'position',[0.13 0.11 0.5 0.203]);
...
try
	...
	set( get(gca, 'ylabel'), 'string', 'Power 10*log_{10}(\muV^{2}/Hz)', 'fontsize', 10); 
	set( get(gca, 'xlabel'), 'string', 'Frequency (Hz)', 'fontsize', 10); 
...
% ---------- my own code ---------------------------------
    ...
    'units','normalized',...
    'position',[0.7 0.19 0.18 0.13],...
    'fontsize',11,...
    'value', 3,...
    'tag','var',...
    'string',{'TMS','EOG','EMG','AEP','Elect','BShift','EKG','Other'},...
    'callback',{@artifact_selected,chanorcomp,disp_pack});

    uicontrol('style','push',...
    'units','normalized',...
    'position',[0.7 0.11 0.18 0.06],...
    'fontsize',12,...
    'string','Close',...
    'callback','close');
    ...
    uicontrol('style','push',...
    'units','normalized',...
    'position',[0.08 0.38 0.18 0.06],...
    'fontsize',12,...
    'string','data',...
    'callback',{@get_data,chanorcomp,EEG});
...
function [] = artifact_selected(varargin)
global I comptype
label = {'TMS','EOG','EMG','AEP','Elect','BShift','EKG','Other'};
obj                     = varargin{1};
chanorcomp              = varargin{3};
disp_pack               = varargin{4}; %Modified MF

if isequal(get(obj,'String'),'Remove')
    set(findobj('tag',num2str(find(I==chanorcomp)),'style','radio'),'value',1)
    
else
%     topos                   = findobj('tag','TOPOS');
%     S                       = guidata(topos);
    comptype(chanorcomp)  = get(obj,'value');
    set(findobj('tag',num2str(find(I==chanorcomp)),'style','radio'),'value',1);
    set(findobj('tag',num2str(find(I==chanorcomp)),'style','push','BackgroundColor',[0.94 0.94 0.94]),'string',label{get(obj,'value')})
end
set(disp_pack.tg,'BackgroundColor','red')
guidata(disp_pack.tg,disp_pack)
close 

function [] = get_data(varargin)
% global I comptype
chanorcomp    = varargin{3};
EEG           = varargin{4};
win           = EEG.times>-0.150 & EEG.times<0.500;
%data          = eeg_getdatact(EEG,'rmcomps',setdiff(1:min(size(EEG.icawinv)),chanorcomp));
%figure;  plot(EEG.times(win),mean(data(:,win,:),3))
data = eeg_getdatact(EEG, 'component', chanorcomp);
% figure;  plot(EEG.times(win),mean(data(:,win,:),3))
eegplot(data,'title',['ICA Component ' num2str(chanorcomp)])
% title(['ICA Component ' num2str(chanorcomp)])
% xlabel('Time(ms)')
% ylabel('Amplitude (mV)')
% xlim([-0.150 0.500])
